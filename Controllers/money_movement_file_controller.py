...
class MoneyMovementFileController:
    ...
    def read_file_csv_money_movement(self, data_csv):
        ...
        logger.debug("Iniciando procesamiento del archivo CSV de movimientos de dinero en el controlador read_file_csv_money_movement")
        ...
        try:
            ...
            for mm_csv in data_csv:
                ...
                required_fields = ['amount', 'date_debit', 'reference', 'account_number']
                if not all(field in mm_csv for field in required_fields):
                    logger.error(f"Datos requeridos faltantes en el registro: {mm_csv}")
                    raise ValueError(f"Datos requeridos faltantes en el registro: {mm_csv}")
                ...
                logger.debug(f"creando payload para el movimiento de dinero con datos: {mm_csv}")
                # Crear payload para el movimiento de dinero
                movement = {
                    "source_id": SOURCE_ID,
                    "destination_id": mm_csv['destination_id'],
                    "amount": amount,
            ...
            if not money_movements:
                ...
                return jsonify({"error": "No se encontraron movimientos válidos para procesar"}), 400
