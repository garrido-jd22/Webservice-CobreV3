...
def set_money_movement():
    try:
        ...
        print("Iniciando procesamiento del archivo CSV de movimientos de dinero")
        print(request.files)
            ...
            for columna in columnas_esperadas:
                ...
                if columna == "amount":
                    try:
                        ...
                        errores.append(
                            f"El campo 'amount' debe ser numérico (puede tener decimales) en la fila {idx}."
                        )
                        logger.error(
                            f"El campo 'amount' debe ser numérico (puede tener decimales) en la fila {idx}."
                        )
                    ...
                    try:
                        ...
                        if fecha_debito < datetime.now().date():
                            errores.append(
                                f"El campo 'date_debit' tiene una fecha menor a la fecha actual en la fila {idx}."
                            )
                            logger.error(
                                f"El campo 'date_debit' tiene una fecha menor a la fecha actual en la fila {idx}."
                            )
                        ...
                        errores.append(
                            f"El campo 'date_debit' tiene un formato de fecha inválido en la fila {idx}."
                        )
                        logger.error(
                            f"El campo 'date_debit' tiene un formato de fecha inválido en la fila {idx}."
                        )
        ...
        logger.debug(
            "Todas las columnas requeridas están presentes y validadas correctamente enviando el archivo al controlador"
        )
